...
- **首页欢迎弹窗**：进入游戏即弹出「学号/昵称」输入窗，支持填写绑定或「跳过」以游客身份游玩（已绑定则不再打扰）。
- **跨端实时同步（WebSocket 多人状态同步）**：手机 / 平板 / 电脑通过「学号绑定」共享同一份学习档案；底层基于 Supabase Realtime（WebSocket 通道），任一设备数据变更，其他在线设备即时收到推送并刷新，教师后台可实时看到全员状态。
- **现代深色 UI（大学生审美）**：深色玻璃拟态 + 霓虹渐变 + 精致圆角与柔和阴影，告别儿童化视觉，更符合大学生审美偏好。
- **数据档案模块**：
  - 🔄 同步状态指示（云端 / 同步中 / 本地模式）
